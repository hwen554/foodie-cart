       ...
       {reviewList?reviewList.map((review,index)=>(
            <div key={index} className='flex gap-5 items-center border rounded-lg p-5'>
                ...
                <div>
                    ...
                    <h2 className='text-sm'><span className='font-medium'>{review.userName} </span>at {moment(review.publishedAt).format('DD-MM-yyyy')}</h2>
                </div>
            </div>
       )):
       [1,2,3,4].map((item,index)=>(
          <div className='h-[10 0px] w-full bg-slate-200 animate-pulse'>
